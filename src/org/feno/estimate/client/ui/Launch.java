...
public class Launch implements EntryPoint {
	...
	private TwoCompartment twoCompModel = new TwoCompartment(new Uniform(0, 100), 2, 0.75, new Uniform(-5000, 5000),
			800, 75, new Uniform(-500, 500), 5, 7.5);
	private final PopupPanel popup = new PopupPanel(false, true); // Create a
																	// modal
																	// dialog
																	// box that
																	// will not
	...
	private void demoCalculate() {
		// HashMap<String, PosteriorSample> paramSamples =
		// twoCompModel.samplePosteriorToo(steps, mtRng);
		// String text = "";
		// for(Parameter param : twoCompModel.getParameters()) {
		// text += "Param " + param.getName() + "Mean " +
		// paramSamples.get(param.getName()).getSampleStats().getMean();
		// }
		...
		chartLoader.loadApi(new Runnable() {
			...
			public void run() {
				...
				Scheduler.get().scheduleDeferred(new ScheduledCommand() {
					...
					public void execute() {
						for (Parameter param : twoCompModel.getParameters()) {
							currentHistograms.get(param.getName()).draw(createDataTable(param.getName()),
									currentHistogramOptions.get(param.getName()));
						}
						Scheduler.get().scheduleDeferred(new ScheduledCommand() {
							@Override
							public void execute() {
